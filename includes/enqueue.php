...
function ai_agent_enqueue(){
    ...
    */
    ...
    /*
    سوال‌هایی که مدیر خودش نوشته، جای همه‌چیز را می‌گیرند.

    هر فروشگاه سوال‌های خودش را دارد — یکی محصول پرفروشی دارد که همه
    درباره‌اش می‌پرسند، دیگری شرایط مرجوعی. کسی که وقت گذاشته و سوالش
    را نوشته، همان را می‌خواهد، نه اینکه ته یک فهرست خودکار بنشیند.

    prompt همان label است: مدیر جمله را برای بازدیدکننده نوشته و
    بازنویسی‌اش توسط ما یعنی مدل چیز دیگری می‌شنود از آنچه روی دکمه
    نوشته شده.
    */
    ...
    /*
    پیشنهادهای خودکار — فقط وقتی مدیر خودش چیزی ننوشته باشد.

    لحن سؤال‌ها محاوره‌ای است، نه اداری: این‌ها را بازدیدکننده می‌خواند و
    قرار است حس کند می‌شود مثل آدم حرف زد، نه اینکه دارد فرم پر می‌کند.

    فقط پیشنهادی ساخته می‌شود که مدیر داده‌اش را پر کرده باشد؛
    «شماره تماس پشتیبانی چیه؟» روی سایتی که شماره‌ای ثبت نکرده،
    سؤالی است که دستیار جوابش را ندارد.
    */
    if (empty($starters)) {
        ...
        if (!empty($settings['organization_name']) || !empty($settings['business_description'])) {
            $starters[] = array(
                'label'  => 'اصلاً شما چه کاری می‌کنید؟',
                'prompt' => 'درباره‌ی این مجموعه و کاری که انجام می‌دهد توضیح بده.',
            );
        }

        /*
        سؤال ساعت کاری و هزینه‌ی ارسال به هیچ فیلدی در تنظیمات وصل نیست —
        جوابشان از محتوای ایندکس‌شده‌ی خود سایت می‌آید. برای فروشگاه‌ها
        پرتکرارترین دو سؤالی هستند که پشتیبانی می‌گیرد، پس وقتی ووکامرس
        فعال است پیشنهاد می‌شوند.
        */
        $starters[] = array(
            'label'  => 'ساعت کاری‌تون چطوره؟',
            'prompt' => 'ساعت کاری و روزهای پاسخ‌گویی شما چطور است؟',
        );
        if (class_exists('WooCommerce')) {
            $starters[] = array(
                'label'  => 'هزینه و زمان ارسال چقدره؟',
                'prompt' => 'هزینه‌ی ارسال چقدر است و سفارش چند روزه به دستم می‌رسد؟',
            );
        }
    }
