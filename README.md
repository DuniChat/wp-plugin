...
- **پیام صوتی**: ضبط در مرورگر با نمایش زندهٔ میله‌های فرکانسی، تبدیل به متن روی سرور (Whisper)
- **سوال‌های پیشنهادی شروع**، قابل تنظیم توسط مدیر (یا ساخت خودکار از روی تنظیمات)
...
### 🤖 ربات پشتیبانی بله
- اتصال ربات با توکن BotFather بله
- همان دستیار سایت، با همان اطلاعات و همان مدل، داخل بله
- پشتیبانی از **متن، عکس و پیام صوتی** (ویس به متن تبدیل می‌شود)
- منو و دکمه‌های شیشه‌ای؛ لینک محصولات زیر پاسخ می‌آید
- انتقال گفت‌وگوی سایت به بله با یک کد شش‌رقمی

> تلگرام پشتیبانی نمی‌شود: سرورهای دانی‌چت داخل ایران هستند و به
> `api.telegram.org` دسترسی ندارند، پس ربات تلگرام توکن را می‌پذیرفت ولی
> هیچ‌وقت به کسی جواب نمی‌داد.

### 🛒 اتصال به ووکامرس
- پاسخ به «سبد خریدم چیه؟» و «قبلاً چی سفارش دادم؟» برای کاربرِ لاگین‌کرده
- افزودن/حذف محصول و تغییر تعداد در سبد، از داخل چت
- کاربر لاگین‌نکرده به ورود راهنمایی می‌شود، نه پاسخ ساختگی
- شناسهٔ کاربر وردپرس هرگز از سایت خارج نمی‌شود (توکن موقت)

### 📚 اطلاعات اضافه و پرسش‌وپاسخ
- آپلود **اکسل، CSV، PDF، متن و عکس** به‌عنوان دانش دستیار
- تعریف پرسش‌وپاسخ‌های آماده («اگر این را پرسیدند، این را بگو»)
- ایندکس روی همان پایگاه دانش سایت، کنار محتوای وردپرس

### 🎧 انتقال به پشتیبان انسانی
- تشخیص خودکار نیاز به Escalation توسط مدل
- پنل مدیریت ‌سشن‌ها با فیلتر وضعیت و **هزینه‌ی هر گفت‌وگو**
- ارسال پاسخ دستی و بستن گفتگو از پیشخوان وردپرس
- پاسخ پشتیبان به گفت‌وگوهای منتقل‌شده، در همان پیام‌رسان تحویل می‌شود
...
├── 🌐 api-extras.php      # تماس‌های ربات‌ها و پایگاه دانش دستی (روی یک هلپر عمومی)
├── 🔁 ajax-extras.php     # هندلرهای AJAX ربات‌ها، اسناد، پرسش‌وپاسخ، انتقال گفت‌وگو و تبدیل صوت
├── 🛒 shop-bridge.php     # مسیر REST سبد خرید و سفارش‌های ووکامرس (با رمز مشترک)
...
| `GET  /api/v1/chat/transfer-options` | پیام‌رسان‌هایی که گفت‌وگو به آن‌ها منتقل می‌شود |
| `POST /api/v1/chat/sessions/{id}/transfer` | صدور کد و انتقال گفت‌وگو به تلگرام/بله |
| `GET/PUT/DELETE /api/v1/bots` | مدیریت ربات تلگرام و بله |
| `GET/POST /api/v1/knowledge/documents` | اسناد دستی (متن یا فایل) |
| `POST /api/v1/knowledge/documents/upload` | آپلود اکسل، CSV، PDF، متن یا عکس |
| `POST /api/v1/chat/transcribe` | تبدیل پیام صوتی ضبط‌شده به متن |
| `GET/POST /api/v1/knowledge/qa` | پرسش‌وپاسخ‌های آماده |
| `POST /api/v1/knowledge/reindex` | ایندکس دوباره‌ی کل دانش دستی |
